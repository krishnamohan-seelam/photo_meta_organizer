...
def _photo(file_hash: str, captured_at) -> ImageMetadata:
    return ImageMetadata(
        ...
        file_info=ImageFileInfo(
            name=f"{file_hash}.jpg",
            path=f"/p/{file_hash}.jpg",
            size_bytes=1,
...
def _search(tmp_path, items, **query):
    """Run a search through a real (TinyDB) repository, since filtering/sorting now
    live in the repository implementation, not in the use case (PMO-09)."""
    import uuid

    repo = TinyDBRepository(str(tmp_path / f"{uuid.uuid4().hex}.json"))
    for item in items:
        repo.save(item)
    try:
        return SearchPhotosUseCase(repo).execute(SearchPhotosQuery(**query))
    finally:
        repo.close()
...
class TestSearchWithMixedDatetimes:
    def test_naive_records_with_aware_bounds_do_not_crash(self, tmp_path):
        """The exact B-04 reproduction."""
        items = [_photo("a", datetime(2026, 3, 1, 12, 0, 0))]
        result = _search(tmp_path, items, date_start=datetime(2026, 1, 1, tzinfo=UTC))
        assert result.total_count == 1

    def test_aware_records_with_naive_bounds_do_not_crash(self, tmp_path):
        items = [_photo("a", datetime(2026, 3, 1, 12, 0, 0, tzinfo=UTC))]
        result = _search(
            tmp_path, items, date_start=datetime(2026, 1, 1), date_end=datetime(2026, 12, 31)
        )
        assert result.total_count == 1

    def test_zulu_bound_is_read_as_wall_clock(self, tmp_path):
        items = [
    ...
    def test_sorting_mixed_naive_aware_and_missing_does_not_crash(self, tmp_path, order):
        items = [
            _photo("naive", datetime(2026, 1, 2)),
            _photo("aware", datetime(2026, 1, 1, tzinfo=UTC)),
            _photo("none", None),
        ]
        result = _search(tmp_path, items, sort_by="captured_at", sort_order=order)
...
class TestRepositoryIndexes:
    def test_mixed_stored_datetimes_build_indexes_and_range_queries(self, tmp_path):
        ...
        repo.save(_photo("aware", datetime(2026, 1, 1, 10, 0, tzinfo=UTC)))

        hits = repo.find_by_date_range(date_start=datetime(2026, 1, 1, tzinfo=UTC))
